...
int main(int argc, char **argv) {
	...
	while (1) {
		...
		printf(
				"please enter virtual machine name:  vm1 vm2 vm3 vm4 vm5 vm6 vm7 vm8 vm9 vm10\n\n");
//		Fgets(vmName, vmNameSize, stdin);
		scanf("%s", vmName);

		printf(
				"\nplease enter 0 or 1 for forced discovery, 1 for forced , 0 for not\n");
		int flag;
		scanf("%d", &flag);
		...
		if (n < 0) {
			if (errno == EINTR) {
				printf("\n\ntimeout!!!\n\n");
				printf(
						"client at node: %s : timeout on response from %s  \n  ",
						sourcevmName, vmName);
			...
			printf("\n\nplease wait for 2 seconds for reply\n\n");
				...
				if (nn < 0) {
					if (errno == EINTR) {
						printf("\n\ntimeout!!!\n\n");
						printf(
								"client at node: %s : timeout again on response from %s    <timestamp>\n  ",
								sourcevmName, vmName);
						printf(
