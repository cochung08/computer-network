...
char* msg_recv(int sockfd, int port) {
	if (port == DEST_PORT) {
		...
		struct hostent *heSource = gethostbyaddr(&dest_ipv4addr,
				sizeof dest_ipv4addr, AF_INET);
		char srcHostName[20];
...
void printReceivingMsg(char*recvline, char* src_hostName,char* dest_hostName )
{
	//
