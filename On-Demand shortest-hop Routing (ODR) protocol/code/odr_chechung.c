...
SELF_DEFINED_PF_Packet* createPF_Packet(int type, char* source_ipAddr,
		int source_port, char* destination_ipAddr, int destination_port,
		int hop_count, char* msg) {
	SELF_DEFINED_PF_Packet *packet = (SELF_DEFINED_PF_Packet *) malloc(
			sizeof(SELF_DEFINED_PF_Packet));
...
void sendMessage(int pfSockfd, char* src_mac, char* dest_mac, int if_index,
	...
	struct hostent *heSource = gethostbyaddr(&source_ipv4addr,
			sizeof source_ipv4addr, AF_INET);
	char srcHostName[20];
	strcpy(srcHostName, heSource->h_name);

	struct in_addr dest_ipv4addr;
	inet_pton(AF_INET, packet->destination_ipAddr, &dest_ipv4addr);
	struct hostent *heDest = gethostbyaddr(&dest_ipv4addr, sizeof dest_ipv4addr,
	AF_INET);
...
//	struct sockaddr_ll socket_address;
	...
	int result = sendto(pfSockfd, buffer, ETH_FRAME_LEN, 0,
...
void broadcast(int PFsockFd, SELF_DEFINED_PF_Packet* sendPF_Packet) {
	...
	for (j = 0; j < INTERFACE_SIZE; j++) {
		...
		if (interfaceArray[j].interfaceNum != -1) {
			...
			unsigned char dest_mac[ETH_ALEN] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
					0xFF };
...
//					int hop_count, char* msg)
			...
			SELF_DEFINED_PF_Packet* copy_Packet = createPF_Packet(
					...
					sendPF_Packet->source_port,
					sendPF_Packet->destination_ipAddr,
	...
	for (i = 0; i < INTERFACE_SIZE; i++) {
		if (interfaceArray[i].interfaceNum != -1) {
			printf("interface num: %d, name: %s\n",
					interfaceArray[i].interfaceNum, interfaceArray[i].macAddr);
		}
	}
}
...
char* getMacAddrByInferFaceNum(int ifaceNum) {
	...
	for (q = 0; q < INTERFACE_SIZE; q++) {
		if (interfaceArray[q].interfaceNum == ifaceNum) {
			...
			//printf("debug:: %d::in getMac() mac: %s\n", __LINE__, interfaceArray[q].macAddr);
...
void sendMessageFromBuffer(int sockfd, SELF_DEFINED_PF_Packet *buffer,
	...
	for (i = 0; i < iterations; i++) {
		...
		time(&curr_time);
		if (checkAddressNotEmpty(table[dest_node].next_hop)
				!= 0 && table[dest_node].timestamp + stale_time > curr_time) {
...
int main(int argc, char **argv) {
	...
	for (;;) {
		...
		if (FD_ISSET(clientUnixSockfd, &rset)) {
			...
			len = sizeof(cliaddr);
			n = Recvfrom(clientUnixSockfd, mesg, MAXLINE, 0, (SA *) &cliaddr,
					&len);
...
//			Sendto(clientUnixSockfd, "wgat", strlen("wgat"), 0, (SA *) &cliaddr,
//						sizeof(cliaddr));
...
/////////////////////////
			...
			for (; *pptr != NULL; pptr++)
				current_addr = Inet_ntop(hptr->h_addrtype, *pptr, str,
						sizeof(str));
////////
			...
			SELF_DEFINED_PF_Packet *new_packet = createPF_Packet(2,
					(char *) current_addr, sourcePort, destinationAddr,
			...
			time(&curr_time);
			if (checkAddressNotEmpty(table[dest].next_hop) == 0 || table[dest].timestamp + stale_time < curr_time) {
				printf("buffering new packet. trying to find route.\n\n");
				// create rreq message if we don't have a path
				SELF_DEFINED_PF_Packet* packet = createPF_Packet(0,
						local_CanonicalIP, sourcePort, destinationAddr,
		...
		if (FD_ISSET(PFsockFd, &rset)) {
			...
			if (message_type == 0) {
				...
				time(&curr_time);
				if (hop_count
						> table[source_node].hops_to_dest  || table[source_node].timestamp + stale_time > curr_time) {
					printf("debug:: no update: %d\n", __LINE__);
					update_reverse = 0;
				}
				//printf("debug:: %d, id: %d, highest_id: %d\n", __LINE__, id, source_entry.highest_broadcast_id[source_node]);
				...
				if (update_reverse > 0) {
					printf("updating reverse path for node: %d\n\n",
							source_node);
					//memcpy(table[source_node].next_hop, getMacAddrByInferFaceNum(incoming_interface), ETH_ALEN);
					memcpy(table[source_node].next_hop, recvAddr, ETH_ALEN);
					//printf("debug:: %d, empty mac: %d\n", __LINE__, checkAddressNotEmpty(source_entry.next_hop));
					//printf("debug:: %d, empty mac: %d\n", __LINE__, checkAddressNotEmpty(table[source_node].next_hop));
					table[source_node].out_interface = incoming_interface;
					//printf("debug:: %d, interface: %d\n", __LINE__, source_entry.out_interface);
				...
				if (curr_node != dest_node) {
					...
					if (checkAddressNotEmpty(table[dest_node].next_hop)
						...
						broadcast(PFsockFd, packet);
					} else if (checkAddressNotEmpty(table[dest_node].next_hop)
							!= 0  && table[dest_node].timestamp + stale_time > curr_time) {
						if (forced_discovery == 0) {
							time(&table[dest_node].timestamp);
							if (rrep_already_sent != 1) {
								printf(
										"we have a route to dest (intermediate node). sending rrep to node: %d\n\n",
										source_node);
								// XXX: create rrep
								char rrep_msg[30];
								memset(rrep_msg, 0, 30);
								strncpy(rrep_msg, packet->application_msg, 30);
								SELF_DEFINED_PF_Packet* out_packet =
										createPF_Packet(1,
												packet->destination_ipAddr,
												packet->destination_port,
												packet->source_ipAddr,
												packet->source_port, 0,
												rrep_msg);
								// XXX: send rrep
								//printf("debug:: %d\n", __LINE__);
								sendMessage(PFsockFd,
										getMacAddrByInferFaceNum(
								...
								free(out_packet);
								if (seen_source == 0
										|| original_hops_to_source
												> hop_count) {
									printf(
											"first time we saw the source node. flooding.\n\n");
									packet->hop_count = packet->hop_count + 1;
							...
							printf(
									"we are intermediate node, but its forced discovery. flooding..\n\n");
							packet->hop_count = packet->hop_count + 1;
							broadcast(PFsockFd, packet);
						}
					}
					...
					if (rrep_already_sent != 1) {
						printf(
								"we are the dest node. sending rrep to node: %d\n\n",
								source_node);
						// XXX: create rrep
						char rrep_msg[30];
						memset(rrep_msg, 0, 30);
						strncpy(rrep_msg, packet->application_msg, 30);
						SELF_DEFINED_PF_Packet* out_packet = createPF_Packet(1,
								packet->destination_ipAddr,
								packet->destination_port, packet->source_ipAddr,
								packet->source_port, 0, rrep_msg);
						if (forced_discovery == 1) {
							// XXX: set rrep with forced discovery
							printf(
									"rreq was forced discovery. this rrep should also be forced discovery.\n\n");
						...
						free(out_packet);
						if (seen_source == 0
								|| original_hops_to_source > hop_count) {
							printf(
									"first time we've seen source. flooding..\n\n");
							packet->hop_count = packet->hop_count + 1;
							packet->application_msg[0] = '1';
							broadcast(PFsockFd, packet);
						}
				...
				if (dest_node == curr_node) {
					...
					sendMessageFromBuffer(PFsockFd, send_buffer,
							&num_packets_buffered);
					printf("debug:: %d, num packets buffered: %d\n", __LINE__,
							num_packets_buffered);
					...
					printf("we are intermediate node. we should send rrep back to source.\n\n");
					//RoutingEntry dest_entry = table[dest_node];
					packet->hop_count = packet->hop_count + 1;
					...
					sendMessage(PFsockFd,
							getMacAddrByInferFaceNum(
									table[dest_node].out_interface),
							table[dest_node].next_hop,
							table[dest_node].out_interface, packet);
				}
			} else {
				int forced_discovery = packet->application_msg[1] - 48;
				// application message
				...
				if (dest_node == curr_node) {
					...
					if (strcmp(packet->application_msg, "request time") == 0) {
						...
						printf("detected time request for server. passing to server application\n\n");
						int serverSockfd;
						...
						Sendto(serverSockfd, msgSequence, strlen(msgSequence),
								0, (SA *) &servaddr, sizeof(servaddr));

						n = Recvfrom(serverSockfd, recvline, MAXLINE, 0, NULL,
						NULL);
						...
						strncpy(response, "response message", 30); // we need string from server
						SELF_DEFINED_PF_Packet* out_packet = createPF_Packet(2,
								packet->destination_ipAddr,
								packet->destination_port, packet->source_ipAddr,
								packet->source_port, 0, recvline);
						send_buffer[num_packets_buffered++] = *out_packet;
					} else {
						printf("detected time response from server. passing to client application\n\n");
//						printf("5678testport\ntestport\ntestport\ntestport\n %d",packet->destination_port);
		...
		/////////////////////////
					...
					for (; *pptr != NULL; pptr++){
						// sourceIP
						current_addr = Inet_ntop(hptr->h_addrtype, *pptr, str2,
								sizeof(str2));
					}
					...
					rreq_msg[copyTo] = '\0';
		////////
					SELF_DEFINED_PF_Packet* rreq_packet = createPF_Packet(0,
								(const char *)current_addr, sourcePort, packet->destination_ipAddr,
