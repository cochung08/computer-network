...
//			memset(interfaceIPAddressStr, 0, sizeof interfaceIPAddressStr);
...
//				interfaceArray[j].address = interfaceIPAddressStr;
//				interfaceArray[j].networkMask = interfaceNetworkMask;
//				interfaceArray[j].subnetAddress = interfaceSubnetAddress;
...
//The client sends a datagram to the server giving the filename for the transfer. This send needs to be
		...
		//if (Fgets(filename1, MAXLINE, stdin) != NULL) {
			...
			for (;;) {
				...
				if (FD_ISSET(sockfd, &rset)) {
					...
					//  const char *serverAddress = inet_ntop(AF_INET, &servaddr.sin_addr, str, sizeof(str));
					//
					// printf("recvfrom ip %s : %d\n", serverAddress,
...
void dg_cli1(FILE *fp, int sockfd, int max_window_size) {
	...
	pthread_t tid;
	struct Parameter_of_print* para_of_print = malloc(
			sizeof(struct Parameter_of_print));
...
// 		pthread_t tid;
// 		struct Parameter_of_print* para_of_print = malloc(sizeof(struct Parameter_of_print));
// 	para_of_print->recv_base=recv_base;
// 	para_of_print->num_seg_in_buf_pointer=&num_seg_in_buf;
// 	para_of_print->last_seg_acked_pointer=&last_seg_acked;
//
...
/* currently prints everything in the window, and clears everything that is printed,
...
/*
		int curr_time = tv.tv_sec * 1000000 + tv.tv_usec + sleeptime;
...
static void after_time_wait(int signo) {
	return;
}
