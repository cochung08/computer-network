			...
			serverAddress = (char*) malloc(
					strlen(
							Sock_ntop_host(getInterfaceSA,
									sizeof(*getInterfaceSA))) * sizeof(char));
			;
...
void mydg_echo(int listenningSockfd, struct sockaddr_in * listenningSA,
		struct Interface* interfaceArray) {
			...
			// 	int off=0;
			//
			// 							if (setsockopt(interfaceArray[hh].sockFd, SOL_SOCKET, SO_DONTROUTE,
			// 									&off, sizeof(off)) < 0) {
	...
	*connectionSA = *listenningSA;
	...
	while (success == 0) {
		...
		n = Recvfrom(listenningSockfd, filename, MAXLINE, 0, (SA *) &pcliaddr,
				&len);
		...
		if (strcmp(connectionAddress, "127.0.0.1") == 0) {
			...
			printf("server and client are in the same localhost, loopback address is used\n");
		} else {
			...
			for (w = 0; w < FD_SETSIZE; w++) {
				if (interfaceArray[w].sockFd > 0) {
					if (strcmp(connectionAddress, interfaceArray[w].address)
							== 0) {
						int networkMaskNum = getNetworkMaskNum(
								interfaceArray[w].networkMask);

						int tmpLong = getLongest(interfaceArray[w].address,
								clientIP);
						...
						if (tmpLong >= networkMaskNum && tmpLong > longestInt) {
							...
							if (setsockopt(connectionsSocketFd, SOL_SOCKET, SO_DONTROUTE,
									&on, sizeof(on)) < 0) {
								printf("error setting socket option: %s\n",
										strerror(errno));
								exit(1);
		...
		for (;;) {
			...
			if ((t = select(time_maxfd, &timeout_fdset, NULL, NULL, NULL))
					< 0) {
				if (errno == EINTR) {
					continue;
				} else {
			...
			if (FD_ISSET(connectionsSocketFd, &timeout_fdset)) {
				...
				printf("parent server %d, datagram from %s", getpid(),
						Sock_ntop((SA*) &pcliaddr, len));
				printf(", to %s\n\n",
						Sock_ntop((SA*) listenningSA,
								sizeof(*((SA*) listenningSA))));

				Sendto(connectionsSocketFd, acknowledgement, t, 0,
						(SA*) &pcliaddr, len);
	...
	if ((pid = Fork()) == 0) { /* child */
		...
		while (sent_complete == 0) {
			...
			/* if timer goes off, the segment at the base of the sliding window is sent
			 if the window is empty, send a dummy segment */

			if ((nready = select(connectionsSocketFd + 1, &listen_set, NULL,
			NULL, &tv)) < 0) {
				if (errno == EINTR) {
					continue;
				} else {
			...
			if (nready == 0) {
				...
				if (1) {
					...
					while (seg_to_send >= 0) {
						...
						seg.timestamp = get_time.tv_sec * 1000000
								+ get_time.tv_usec;
						if (seg_to_send == 0) {
						...
						SlidingWindow* nodePtr = send_base;
						while (nodePtr->nextSeg != NULL) {
							if (nodePtr->nextSeg->seg.sequence_num
									== seg.sequence_num) {
								//num_seg_in_buf++;
								break;
							}
						...
						seg_to_send--;
						/*
						 n = Recvfrom(connectionsSocketFd, mesg, MAXLINE, 0, (SA*) &pcliaddr,
						 &len);
