...
void printArpTable(){
	...
	for(i = 0; i < numEntries; i++){
		...
		for(j = 0; j < ETH_ALEN; j++){
			printf("%.2x%s", arpEntry.macAddr[j] & 0xFF, (j == ETH_ALEN - 1) ? "\n" : ":");
		}
...
//	struct sockaddr_ll socket_address;
	...
	int result = sendto(pfSockfd, buffer, ARP_LEN, 0,
...
void recvArpMessage(int PFsockFd, ArpMessage *arpMessage){
	...
	//printf("\n\nline: %d, sizeof buf: %ld, sizeof message: %ld\n", __LINE__, sizeof(buffer), sizeof(ArpMessage));
	memcpy(arpMessage, buffer + 14, sizeof(ArpMessage));
	//printf("\nline: %d, %d:%d\n", __LINE__, (int) arpMessage->op[0], (int) arpMessage->op[1]);
	printArpMessage(buffer);
}

void ip_ntop(unsigned char *ip, char *ipPresentation){
	sprintf(ipPresentation, "%d.%d.%d.%d", (int) ip[0], (int) ip[1], (int) ip[2], (int) ip[3]);
}
	...
	for(;;){
		...
		if(FD_ISSET(arpUnixSockFd, &rset)){
			...
			if(index == -1){
				...
				memcpy((void *)&arpMessage.sender_mac, (void *)&interfaceArray[0].macAddr, ETH_ALEN);
				arpMessage.op[1] = 0xFF;
				//printPacketBytes(&arpMessage, sizeof(ArpMessage));
				printf("sending broadcast arp message\n");
				sendArpMessage(PFsockFd, (char *)&arpMessage.sender_mac, (char *)&arpMessage.target_mac
		...
		if(FD_ISSET(PFsockFd, &rset)){
			...
			if(arpMessage.op[0] == 0x00 && arpMessage.op[1] == 0xFF){
				...
				int foundIndex;
				if((foundIndex = checkEntryExist(arpMessage.sender_ip, arpMessage.sender_mac)) > 0){
				...
				//printf("line: %d, we are dest: %d\n", __LINE__, checkIsDestNode(arpMessage.target_ip));
				if(checkIsDestNode(arpMessage.target_ip)){
					...
					if(foundIndex == -1){
						...
						memcpy((void *)replyMessage.sender_ip, (void *)arpMessage.target_ip, IP_ALEN);
						memcpy((void *)replyMessage.sender_mac, (void *)interfaceArray[0].macAddr, ETH_ALEN);
						memcpy((void *)replyMessage.target_ip, (void *)arpMessage.sender_ip, IP_ALEN);
						memcpy((void *)replyMessage.target_mac, (void *)arpMessage.sender_mac, ETH_ALEN);
						printf("we will send the reply back to the sender\n");
						sendArpMessage(PFsockFd, (char *)&replyMessage.sender_mac, (char *)&replyMessage.target_mac
				...
				ip_ntop(arpMessage.sender_ip, ipPresentation);
				int foundIndex;
				//printf("line:%d, index of ip: %d\n", __LINE__, indexOfIP(ipPresentation));
				if((foundIndex = indexOfIP(ipPresentation)) >= 0){
					printf("updating entry using the reply\n");
					// if entry exists, we modify it
					//ArpEntry arpEntry = arpTable[foundIndex];
					memcpy((void *)arpTable[foundIndex].macAddr, (void *)arpMessage.sender_mac, ETH_ALEN);
					...
					if(arpTable[foundIndex].conn_fd > -1){
						printf("we have a tour client who wants the reply. let's send it\n");
						Writen(arpTable[foundIndex].conn_fd, arpTable[foundIndex].macAddr, ETH_ALEN);
