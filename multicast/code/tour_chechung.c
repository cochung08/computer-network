		...
		if (strcmp(hwa->if_name, "eth0") == 0) {
			...
			for (p = 0; p < INTERFACE_SIZE; p++) {
				if (interfaceArray[p].interfaceNum == -1) {
					...
					memcpy(interfaceArray[p].ipAddr, local_CanonicalIP,
					IPADDR_SIZE);
...
//	struct sockaddr_ll socket_address;
	...
	//printf("line: %d, %d:%d", __LINE__, (int) arpMessage->op[0], (int) arpMessage->op[1]);
...
void ip_ntop(unsigned char *ip, char *ipPresentation) {
	sprintf(ipPresentation, "%d.%d.%d.%d", (int) ip[0], (int) ip[1],
			(int) ip[2], (int) ip[3]);
}
...
void Get_LOCAL_ADDR() {
	...
	struct hostent * hp = (struct hostent *) gethostbyname(
...
PayLoad receiveRTpacket(int rtsockfd) {
	...
	Recvfrom(rtsockfd, (void *) buff, IP_WHOLE_PACK_SIZE, 0, (SA *) &addr,
			&len);
...
//multicast
		...
		multiSendSockfd = Udp_client(multicastAddr, multicastPort,
				(void **) &sasend, &salen);
			...
			if (ping_switch_ == 1) {
				...
				if (ping_counter_array[previous_vmNum_static] >= 5) {
					if (if_last_node == 1) {
						...
						char msgSequence[MAXLINE];
						sprintf(msgSequence, "%s %s %s", "<<<<< This is node ",
								localHost,
...
//				send_v4(pfsockfd, pgsockfd, previous_vmNum);
