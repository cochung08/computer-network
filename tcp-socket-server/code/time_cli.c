...
// if connot connect to server, then it send message to parent window and then close the child thread
...
void*
str_cli_time1(FILE *fp, int sockfd, char* pfdStr) {
	...
	while (1) {
		...
		//read time info from the server. If server crashes,read socket return 0 and  child tells parent window that server crashs
